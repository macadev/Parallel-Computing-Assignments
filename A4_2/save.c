...
void parallelized_RREF_helper(double *matrix, int rows, int columns, int rank, int rowsPerProcess);
...
void find_num_of_rows_and_cols(char *filename, int *rows, int *columns, int npes, int rowsPerProcess);
...
int main(int argc, char * argv[])
{
  ...
  if (rank == 0) {
    ...
    if (npes == 1) {
      A = read_user_matrix_from_file(argv[1], &rows, &columns);
    	...
    	find_num_of_rows_and_cols(argv[1], &rows, &columns);
  ...
  if (npes == 1) {
    ...
    read_matrix_chunk(argv[1], matrix_chunk, rows, columns, rank);
...
void read_matrix_chunk(char *filename, int rows, int columns, double *matrix_chunk) {
	FILE *file;
...
void parallelized_RREF(double *linear_A, double* matrix_chunk, int columns, int rowsPerProcess, int rowsForLastProcess, 
  ...
  if (rank < npes - 1) {
    parallelized_RREF_helper(matrix_chunk, rowsPerProcess, columns, rank, rowsPerProcess);
  } else {
    parallelized_RREF_helper(matrix_chunk, rowsForLastProcess, columns, rank, rowsPerProcess);
  }
  ...
  // if (rank == 0) {
  //   printf("Divided\n");
  //   print_linear_matrix(linear_A, rowsPerProcess * (npes - 1) + rowsForLastProcess , columns);
  // }
}
...
void find_num_of_rows_and_cols(char *filename, int *rows, int *columns, int npes, int rowsPerProcess) {
	...
	file = fopen(filename, "r");
  int read_pointer = 0;
  int chunk_pointers[] = malloc(*npes * sizeof(int));
  int chunk_row = 0, chunk_counter = 0; 

  line_pointers[line_num] = 0;
	...
	*columns = 1;
	...
	while(!feof(file)) {
	  ...
	  if (c == '\n') {
      if (chunk_row == 0) {
        chunk_pointers[chunk_counter] = ftell();
        chunk_counter++;
      }
      chunk_row++
      if (chunk_row == rowsPerProcess) {
        chunk_row = 0;  
      }
	    (*rows)++;
	    columns_known = 1;
